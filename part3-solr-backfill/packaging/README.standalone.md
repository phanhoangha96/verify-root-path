...
CLI: Oracle NEW/LEGACY → Solr `searchText` + field riêng (`docCode`, `quote`, `bookNumber`, `instruction`, `comment`, ...) → Excel/JSON.
...
Binary cũ thiếu `cryptography` (oracledb thin mode). `pip install` trên server không giúp. Cần gói build lại bằng `python build/build_solr_backfill_dist.py --linux`.
