...
CLI one-shot: đọc văn bản từ Oracle (NEW và/hoặc LEGACY) → ghi `searchText` và field riêng (`docCode`, `quote`, `outsidePublisherName`, `otherReceivePlaces`, `bookNumber`, `instruction`, `comment`) lên Solr → xuất Excel/JSON.
