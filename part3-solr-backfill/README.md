...
CLI one-shot: đọc văn bản từ Oracle (NEW và/hoặc LEGACY) → ghi `searchText` lên Solr → xuất Excel/JSON.
...
- `--source ALL` (mặc định) yêu cầu **cả hai** khối đã điền password + DSN.
