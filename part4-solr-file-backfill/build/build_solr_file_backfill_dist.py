#!/usr/bin/env python3
"""Build dist/solr-doc-file-backfill (+ zip) for DevOps. Part 4 only.
...
DIST_ROOT = ROOT / "dist"
PACKAGE_NAME = "solr-doc-file-backfill"
PACKAGE_DIR = DIST_ROOT / PACKAGE_NAME
PACKAGING = ROOT / "packaging"
BINARY_NAME = PACKAGE_NAME

PYTHON_FILES = [
    "solr_doc_file_backfill.py",
    "solr_file_config.py",
    "solr_file_db.py",
    "solr_file_solr.py",
    "solr_file_download.py",
    "solr_file_extract.py",
    "solr_file_report.py",
]
...
def _pyinstaller_cmd(work_dir: Path, *, onefile: bool) -> list[str]:
    ...
    cmd.extend(
        [
            ...
            "--specpath",
            str(work_dir),
            str(work_dir / "solr_doc_file_backfill.py"),
        ]
    )
    return cmd
...
def _build_linux_docker(*, onefile: bool) -> None:
    if not _docker_available():
        raise SystemExit(
            ...
            "  python build/build_solr_file_backfill_dist.py --standalone"
        )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build Part 4 dist zip for DevOps.")
    parser.add_argument(
        "--standalone",
        action="store_true",
    ...
    if args.linux:
        print("Linux amd64 binary. DevOps: unzip, cp .env.example .env, ./solr-doc-file-backfill")
    elif args.standalone:
        print(
            f"Standalone binary for {sys.platform}. "
            "PRD Linux needs: python build/build_solr_file_backfill_dist.py --linux"
        )
    else:
        print(
            "Source package (Python required). For no-Python PRD: "
            "python build/build_solr_file_backfill_dist.py --linux"
        )
    return 0
