...
| **3. Solr metadata backfill** | [`part3-solr-backfill/`](part3-solr-backfill/) | Đọc Oracle NEW + LEGACY → ghi Solr `searchText` + `instruction`/`comment` → Excel/JSON |
| **4. Solr file content backfill** | [`part4-solr-file-backfill/`](part4-solr-file-backfill/) | Đọc Oracle + file (file-service / disk) → Tika → bỏ dấu tiếng Việt → ghi Solr `fileContent` → Excel/JSON |
